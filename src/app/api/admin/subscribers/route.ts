...
export async function GET(req: Request) {
  ...
  const { data: subs, error } = await admin.from("subscribers").select("*").order("created_at", { ascending: false }).limit(200);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });

  const ids = (subs ?? []).map((s) => s.id);
  const { data: keys } = ids.length
    ? await admin.from("access_keys").select("subscriber_id,key,expired_at,last_login_at").in("subscriber_id", ids)
...
export async function POST(req: Request) {
  ...
  const { data: existing } = await admin.from("subscribers").select("*").eq("email", normalizedEmail).maybeSingle();

...
const ACCESS_KEY_STORAGE_KEY = "RJI-access-key";
...
export default function Home() {
  ...
  useEffect(() => {
    const saved = typeof window !== "undefined" ? window.localStorage.getItem("RJI-theme") : null;
  ...
  useEffect(() => {
    if (typeof window !== "undefined") window.localStorage.setItem("RJI-theme", theme);
  }, [theme]);
    ...
    if (latestSignal && latestSignal.id !== lastSeenSignalIdRef.current && latestSignal.status === "active") {
      ...
      if (!notifiedEventKeysRef.current.has(eventKey)) {
        ...
        pushLiveAlert({
          id: eventKey,
          kind: "signal",
          title: `New ${latestSignal.mode.toUpperCase()} The Move`,
      ...
      if (next.status === "active") {
        ...
        if (!notifiedEventKeysRef.current.has(eventKey)) {
          ...
          pushLiveAlert({
            id: eventKey,
            kind: "signal",
            title: `New ${next.mode.toUpperCase()} The Move`,
  ...
  if (!authorized) {
    ...
    return (
      <main className={`grid min-h-screen place-items-center px-4 ${loginDark ? "rji-shell" : "light-theme bg-[#e2e8f0] text-[#0f172a]"}`}>
        <section className={`scanlines w-full max-w-md rounded-xl p-6 ${
          loginDark
            ? "border border-[#d4af37]/35 bg-[#120f1a]/90 shadow-[0_0_40px_rgba(212,175,55,0.15)]"
          ...
          <div className="mb-3 flex justify-end gap-2">
            <button
              ...
              className={`inline-flex items-center gap-1 rounded border px-3 py-1.5 text-[10px] font-bold ${
                loginDark
                  ? "border-[#d4af37]/40 bg-[#301934]/60 text-[#d4af37] hover:bg-[#3f2146]"
                  : "border-[#0f172a]/20 bg-white text-[#0f172a] hover:bg-slate-100"
              }`}
            >
          ...
          <div className="relative">
            <input
              ...
              className={`w-full rounded border px-3 py-2 pr-11 outline-none ring-[#d4af37]/30 focus:ring ${
                loginDark
                  ? "border-[#d4af37]/35 bg-[#0f0d16] text-[#e5e7eb]"
                  : "border-[#0f172a]/25 bg-white text-[#0f172a]"
              }`}
            ...
            <button
              ...
              className={`absolute right-2 top-1/2 -translate-y-1/2 rounded p-1 ${
                loginDark ? "text-[#c0c0c0] hover:bg-[#d4af37]/10" : "text-slate-600 hover:bg-slate-100"
              }`}
          ...
          <button
            ...
            className={`mt-5 w-full rounded border py-2 transition disabled:opacity-50 ${
              loginDark
                ...
                : "border-[#d4af37]/40 bg-[#301934] text-[#d4af37] hover:bg-[#3f2146]"
            }`}
          >
  ...
  return (
    <main className={`min-h-screen px-3 py-4 sm:px-6 ${isDark ? "rji-shell" : "light-theme bg-[#e2e8f0] text-[#0f172a]"}`}>
            ...
            <div className="flex flex-wrap items-start justify-between gap-3">
              ...
              <div className="exec-action-group flex flex-wrap items-center gap-2">
                ...
                <button onClick={() => setTheme((prev) => (prev === "dark" ? "light" : "dark"))} className="exec-head-btn inline-flex items-center justify-center gap-1 rounded border border-emerald-400/40 px-2 py-1 text-[10px] hover:bg-emerald-500/20">
                  ...
                  {isDark ? "Light" : "Dark"}
                </button>                <button onClick={refreshNow} disabled={isRefreshing} className="exec-head-btn inline-flex items-center justify-center gap-1 rounded border border-emerald-400/40 px-2 py-1 text-[10px] hover:bg-emerald-500/20 disabled:opacity-50">
            ...
            <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
              ...
              <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap sm:items-center sm:justify-end md:pt-1">
                <button onClick={() => setTheme((prev) => (prev === "dark" ? "light" : "dark"))} className="inline-flex items-center justify-center gap-1 rounded border border-emerald-400/40 px-2 py-1 text-[10px] hover:bg-emerald-500/20">
                  {isDark ? <Sun size={12} /> : <Moon size={12} />}
                  {isDark ? "Light" : "Dark"}
                </button>                <button onClick={refreshNow} disabled={isRefreshing} className="inline-flex items-center justify-center gap-1 rounded border border-emerald-400/40 px-2 py-1 text-[10px] hover:bg-emerald-500/20 disabled:opacity-50">
                  {isRefreshing ? "Syncing..." : "Refresh"}
                </button>
          ...
          isExecutive ? (
                ...
                <div className="exec-signal-panel rounded-2xl border border-emerald-500/30 p-5">
                  <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                    ...
                    <div className="rounded-full border border-emerald-500/40 bg-emerald-500/10 px-6 py-2 text-sm font-bold text-emerald-200">
                      EXECUTE {activeSignal?.type?.toUpperCase() ?? "THE PLAY"}
                    </div>
                  </div>
            ...
            <section className="space-y-4">
              <div className="rounded border border-emerald-500/30 bg-emerald-500/5 px-4 py-3 text-sm">
                <span className="uppercase tracking-[0.2em] text-emerald-300/75">Next The Move</span>
              ...
              {activeSignal && (
                <div className="rounded border border-emerald-500/30 px-4 py-2 text-sm">
                  <span className="uppercase tracking-[0.2em] text-emerald-300/75">The Play Direction</span>
...
function Card({ title, value, meta, highlight = true, className = "", copyValue }: { title: string; value: string; meta?: string; highlight?: boolean; className?: string; copyValue?: string }) {
  ...
  return (
    ...
    >
      ...
      <p className={`text-[2.35rem] font-extrabold tracking-tight ${highlight ? "text-emerald-300" : "text-red-400"}`}>
        {pipsMatch ? (
          <span className="inline-flex items-end gap-2">
            <span>{pipsMatch[1]}</span>
            <span className="text-base font-semibold lowercase tracking-normal opacity-95">pips</span>
          </span>
        ) : (
          value
        )}

...
export default function RegisterPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  useEffect(() => {
    const saved = typeof window !== "undefined" ? window.localStorage.getItem("RJI-register-theme") : null;
    if (saved === "dark" || saved === "light") setTheme(saved);
  }, []);

  useEffect(() => {
    if (typeof window !== "undefined") window.localStorage.setItem("RJI-register-theme", theme);
  }, [theme]);
  ...
  const downloadKeyTxt = () => {
    ...
    a.href = url;
    a.download = `RJI-access-key-${result.access_key}.txt`;
      ...
      <section className={`w-full max-w-xl rounded-2xl border p-6 ${isDark ? "border-slate-700 bg-slate-900/70" : "border border-[#0f172a]/20 bg-[#f8fafc] shadow-[0_10px_30px_rgba(15,23,42,0.14)]"}`}>
        <div className="mb-4 flex items-center gap-3">
          <Image
            ...
            width={56}
            height={56}
            className={`h-14 w-14 object-contain ${isDark ? "mix-blend-screen" : ""}`}
          />
          <p className="text-xs uppercase tracking-[0.16em] text-[#d4af37]">Member Access Registration</p>
        </div>
        ...
        {pkg && !result && (
          <div className="mt-4 space-y-3">
            <div className={`rounded border px-3 py-2 text-sm ${isDark ? "border-slate-700 bg-slate-950/70 text-slate-200" : "border-[#d4af37]/35 bg-white text-[#1f2937]"}`}>Package: <span className="font-semibold">{pkg.package_name}</span> ({pkg.duration_days} days)</div>
            <input
              required
              autoComplete="name"
